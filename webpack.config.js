  ...
  module: {
    rules: [
      ...
      // only when the issuer is a .js/.ts file, so the loaders are not applied inside html templates
      {
        ...
        ] : [require.resolve('style-loader'), ...cssRules]
      },
      {
        test: /\.css$/i,
        issuer: [{ test: /\.html$/i }],
      ...
      { test: /\.html$/i, loader: require.resolve('html-loader') },
      { test: /\.ts$/, loader: require.resolve("ts-loader") },
      // embed small images and fonts as Data Urls and larger ones as files:
      { test: /\.(png|gif|jpg|cur)$/i, loader: require.resolve('url-loader'), options: { limit: 8192 } },
      { test: /\.woff2(\?v=[0-9]\.[0-9]\.[0-9])?$/i, loader: require.resolve('url-loader'), options: { limit: 10000, mimetype: 'application/font-woff2' } },
      { test: /\.woff(\?v=[0-9]\.[0-9]\.[0-9])?$/i, loader: require.resolve('url-loader'), options: { limit: 10000, mimetype: 'application/font-woff' } },
      // load these fonts normally, as files:
      { test: /\.(ttf|eot|svg|otf)(\?v=[0-9]\.[0-9]\.[0-9])?$/i, loader: require.resolve('file-loader') },
      {
        test: /environment\.json$/i, use: [
          { loader: require.resolve("app-settings-loader"), options: { env: production ? 'production' : 'development' } },
        ]
      },
      ...when(tests, {
        test: /\.[jt]s$/i, loader: require.resolve('istanbul-instrumenter-loader'),
